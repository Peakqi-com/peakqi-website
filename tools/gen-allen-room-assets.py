...
DIFF_T = 15       # 原圖與空房間的色差門檻(三通道相加)
# 34 太高:物件的柔邊與軟陰影低於門檻,就留在底板上不動 —— 元件一擺動,那一圈
# 淡淡的輪廓就自己現形(每一個會動的元件在最大擺幅都看得到)。掃過 34/26/20/15/11:
# 遮罩到 15 都還是平滑成長,11 開始暴衝(檯燈 +27%、海報 +24%、盆栽 +34%),
# 那是淹進牆面的重畫雜訊。15 是「收乾淨」和「不亂吃」之間的最後一格。
