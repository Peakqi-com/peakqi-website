...
export const compare = {
  dims: ['適合情況', '優點', '需要注意', '費用模式'],
  cols: [
    { name: '單一 AI 工具', vals: ['只處理一個明確任務', '上手快', '資料可能仍需人工轉移', '單一工具訂閱'], hot: false },
    { name: '完整客製開發', vals: ['有特殊流程與充足預算', '彈性最高', '前期規格與維護投入較高', '專案報價'], hot: false },
    { name: 'PeakQi 模組化導入', vals: ['希望先驗證,再逐步整合', '起步範圍可控制,模組可累加', '仍需要企業參與流程與資料確認', '導入費＋月費＋使用量費'], hot: true }
  ]
};

// 三種「導入起點」:差異是流程深度,不是公司規模。
// setup/monthly 供價格摘要顯示(此前 {{ fp.setup }} 未定義,是既有 console 警告來源)。
export const plans = [
  { code: 'A', name: 'AI 接客起步方案', tag: '接住詢問', quote: '依需求報價',
    who: '已有穩定詢問量,但團隊花大量時間重複回答、確認需求與安排預約。',
    goal: '先把詢問接住、整理,再交給正確的人。',
    base: '',
    items: ['核准知識查詢', '常見問題回覆與回覆草稿', '需求欄位擷取', '預約引導', '人工轉接', '基本客戶紀錄'],
    excludes: ['完整 CRM 階段管理', '自動跟進序列', '報價與專案管理', '跨部門控制台'],
    upgrade: '當你不只需要回答問題,還需要管理客戶階段、負責人和下一步時,建議進入 B。',
    setup: '第一階段導入費・依範圍評估', monthly: '平台與維運月費・依方案確認',
    featured: false, badge: '' },
  { code: 'B', name: 'AI 業務流程方案', tag: '推進案件', quote: '依需求報價',
    who: '詢問進來後容易停住,需要管理客戶狀態、負責人與後續動作。',
    goal: '讓每一筆詢問都有負責人、目前階段與下一步。',
    base: '包含 A,再加上:',
    items: ['CRM 客戶與案件', '負責人分派', '客戶階段', '跟進提醒', '常見疑慮回覆草稿', '互動紀錄', '基本流程分析'],
    excludes: ['完整報價流程', '專案交付管理', '跨部門權限', '大型資料遷移', '深度客製介面'],
    upgrade: '當報價、交付與跨部門資料也要接進來時,建議進入 C。',
    setup: '第一階段導入費・依範圍評估', monthly: '平台與維運月費・依方案確認',
    featured: true, badge: '建議起步' },
  { code: 'C', name: 'AI 營運整合方案', tag: '整合營運', quote: '依需求報價',
    who: '客服、業務、報價與交付由不同部門或工具負責,需要建立跨流程營運視圖。',
    goal: '將詢問、成交與交付接成同一條可管理的流程。',
    base: '包含 B,再加上:',
    items: ['內容與行銷流程', '報價與提案草稿', '專案交接', '任務與時程', '管理儀表板', '多角色權限', '既有工具串接', '流程與使用數據'],
    excludes: ['超出標準模組的深度客製(獨立列於客製評估)'],
    upgrade: '標準模組無法直接覆蓋時,客製項目、交付範圍與費用會獨立列出。',
    setup: '第一階段導入費・依範圍評估', monthly: '平台與維運月費・依方案確認',
    featured: false, badge: '' }
];
export const planNote = '實際費用依需求範圍與使用情境評估。歡迎預約諮詢,我們會依你的場景給出建議與報價;各產品的詳細價格請見該產品官網。';
// 不做「取代員工」對比:系統處理重複工作,人保留判斷與服務
export const humanCompare = { leftT: '人工重複處理', leftV: '逐筆複製、整理與在工具間轉貼資料', leftU: '', rightT: 'AI 協作流程', rightV: '系統彙整與提醒,人工確認後送出', rightU: '', rightD: 'AI 先處理常見問題,例外轉人工。' };
...
export const usage = [
  { t: '文字類:標準用量包含於月費', d: 'AI 對話、文案、報告與分析的標準用量含在月費內;實際額度與公平使用範圍依方案確認。', c: '#65E0BC' },
  { t: '圖片・影片類:按使用量計費', d: 'AI 場景渲染、風格模擬、短影音等高成本用量,依實際使用量計費。', c: '#3E9BFF' }
];
...
export const faq = [
  ...
  { q: 'AI 使用量怎麼計費?', a: '文字類(對話、文案、報告、SEO 分析)的標準用量含在月費內,實際額度與公平使用範圍依方案確認;圖片影片類依實際使量;圖片與影片類(場景渲染、風格模擬、短影音)按使用量計費,用多少付多少。' },
...
export const faqPricing = [
  { q: '報價主要取決於哪些因素?', a: '啟用的模組數量、流程設定範圍、資料整理程度、既有工具串接、權限角色與是否需要客製介面。Demo 之後會列出具體項目。' },
  { q: '導入費、月費與使用費有什麼差別?', a: '導入費是第一階段的一次性專案費(設定、資料、串接);月費涵蓋系統使用、維運與標準文字 AI 用量;圖片影片等高成本 AI 依實際用量計費。' },
  { q: '三種方案可以後續升級嗎?', a: '可以。A → B → C 是能力累加,升級時保留既有資料與設定,只補上新增模組的導入工作。' },
  { q: '可以只先做一個模組嗎?', a: '可以。建議從最影響營運的一段流程開始,確認有效再擴大。' },
  { q: '既有 LINE、CRM 或表單可以保留嗎?', a: '可以。我們處理的是工具之間原本需要人工搬運的部分;實際串接方式依工具、API 與權限評估。' },
  { q: '什麼情況需要客製開發?', a: '需要串接多套現有系統、資料遷移、多部門權限、特殊審核流程或專屬介面時,客製項目會與標準方案分開評估與報價。' },
  { q: '第一階段通常包含什麼?', a: '目標與範圍確認、模組設定、資料整理、內部測試與上線交接;細項會寫在提案裡。' },
  { q: '媒體用量超過之後怎麼計費?', a: '圖片與影片類依實際使用量計費;導入前會依你的流程先估算範圍,避免意外帳單。' },
  { q: '合作期間與取消方式?', a: '合約期限、最低合作期間與調整條件依方案而定,簽約前會逐條說明。' }
];

// k = 該步的小劇場類型(對應 Solutions.dc.html 的 [data-fanim] CSS);a = 劇場說明;e = 右上角狀態(空字串則不顯示)
export const followupSeq = [
  { d: 'STEP 01', t: '補齊需求:缺的資料由 AI 擬好追問訊息', k: 'fill', a: 'AI 補上缺的那一欄', e: '缺 1 項' },
  { d: 'STEP 02', t: '提供相關案例或資料,幫客戶往下評估', k: 'deck', a: '挑出對應的案例送出', e: '' },
  { d: 'STEP 03', t: '確認客戶反應,更新案件狀態', k: 'reply', a: '客戶回覆・狀態更新', e: '有意願' },
  { d: 'STEP 04', t: '由負責人決定下一步:報價、約訪或結案', k: 'fork', a: '負責人選一條路走', e: '' }
];
...
export const solutionsScenarios = [
  ...
  ], rec: { src: 'LINE 官方帳號', need: '服務流程說明', miss: '想解決的狀況', st: 'AI 可處理', stc: 'ai' },
  ...
  ], rec: { src: 'LINE 官方帳號', need: '預約時段', miss: '無(資料齊全)', st: 'AI 可處理', stc: 'ai' },
  ...
  ], rec: { src: 'LINE 官方帳號', need: '估價', miss: '需求範圍、預算區間', st: '待人工確認', stc: 'h' },
  ...
  ], rec: { src: 'LINE 官方帳號', need: '複雜需求洽談', miss: '細節待專員確認', st: '已轉交負責人', stc: 'h' },
