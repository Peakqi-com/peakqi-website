...
- `allen-room.js` — 工作間的互動層。**背景那個檔副檔名雖然是 .svg,裡面其實是一張 PNG**,
  房間裡的東西沒辦法直接抓來動,所以這一層是照原圖座標重畫一次會動的部分:窗外的捲雲、
  控制台螢幕、牆上小螢幕、檯燈、馬克杯蒸氣、機台指示燈、海報上那隻會眨眼。
  六個地方可以點(窗戶起風、檯燈開關、杯子噴蒸氣、控制台跑資料、機台閃燈、海報眨眼),
  點下去 Allen 會轉頭看過去。座標寫在 `P`,換背景圖就要重量。
  `?roomdebug=1` 會把命中範圍畫出來。

轉身:`assets/allen/turnaround.webp` 是八視角圖切出來的 45°–270° 六格,轉身那一段用它。
**它和正面那隻不是同一版畫的**(量過寬度差 7.2%,胸板與膝蓋高度也對不上),所以規則是
「不讓點陣的正面和向量的正面前後相接」—— 同一個姿勢接在一起會看出換圖,轉身過程中的
姿勢本來就在變,差異被動作吃掉。要完全無縫,得請美術用同一版重出轉身圖。
精靈圖每格都以腳底中心軸對齊(切圖時算的),放進和角色同一個 viewBox,對位就只是算術;
實測換圖瞬間腳底差 1–2px、中心軸零位移。104KB,滑到卡片上(或閒置 8 秒)才載。
