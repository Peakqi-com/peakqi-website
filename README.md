...
- `allen-room.js` — 工作間的互動層,**剪紙動畫式:會動的每一樣東西都是原圖裡那個物件本身**。
  素材來自 `assets/svg/test.svg` —— 美術**手工拆好的 40 個圖層**(每層是彩色 PNG ×
  灰階遮罩)。底板 = 第 00 層「完全空的房間」+ 所有不會動的層;會動的元件就是它自己
  那一層,alpha 是畫出來的不是算出來的 —— **零修補、零門檻,也就零殘影**。
  元件貼回自己的 bbox,繞物理上真正的轉軸動:盆栽從土面搖、洞洞板的板手與起子從掛孔擺、
  馬克杯與檯燈繞底座微晃、按鈕按下去是位移、螢幕不做幾何只把自己的像素疊亮。
  六個地方可以點(窗戶、檯燈開關、馬克杯、海報、控制台、工具排),點下去 Allen 會轉頭
  看過去。`?roomdebug=1` 會把命中範圍畫出來。
  窗外有三朵雲會飄,而且是**完整的一朵** —— 原稿裡四朵都被畫布左緣或窗框的弧切過,
  產生器用「把靠近切口那 70% 鏡射過去再閉合」補全(雲是純白的,所以補的只有輪廓)。
  雲的可見範圍 = 天空那一層減掉城市,**硬邊**:窗框與每一棟建築都會確實擋住它。
  **夜裡的窗外**:建築是暗的剪影,不是霓虹 —— 整片點成螢光色很醜,而且那是把顏色蓋
  上去不是打光。改成「窗戶裡面有光」:窗格自己的像素切成三組貼圖,在網頁端 screen 疊亮
  並各閃各的(整座城市一起明滅會像在呼吸,不像有人在裡面)。
  夜空另外有**星星**與**慢慢升起的月亮**,月亮升得越高,灑進房間的冷月光越多。
  這是全檔唯一「原稿裡沒有、自己畫」的東西 —— 原稿只畫了白天,夜空是空的。
  兩者都畫在**發光層**不是幾何層:幾何層在天色分級之下,白色的月亮會被夜晚的 multiply
  壓成夜空色(× 0.14)等於看不見。也都套同一個天空遮罩,所以窗框與每一棟建築都會擋住。

  另外有三個「只會亮不會動」的地方(牆上小螢幕、頂上藍螢幕、地上機台燈條):不另外切圖,
  直接把底板自己的像素在那塊用 screen 混合疊亮,零新增檔案。光的範圍用軟邊的放射遮罩,
  不用硬邊矩形 —— 矩形在白天疊在同樣亮的底板上剛好消失,天色一暗就整個現形。
  還有一個不定時排程,每 5–12 秒挑一件事做大一點(陣風 / 螢幕跑資料 / 燈條連閃 /
  檯燈閃一下 / 只有工具排晃)—— 全部一起慢慢晃的話畫面沒有起伏。

  唯一的例外是疊圖順序:右上那排層架畫在海報前面,所以它單獨出一張 `front_shelf.webp`
  畫在會動的層之上,其餘 38 層直接烤進底板。
  退化的一項:層架上那盆小盆栽沒有單獨一層(和層架畫在一起),所以它不再會晃。
  美術唯一沒拆掉的是「光」:檯燈打在洞洞板與工作檯上的暖光還畫在那兩層裡(59% / 24%),
  所以關燈時那道光還在;要拿掉得請美術補一張「沒開燈的洞洞板 + 桌面」。
...
- `allen-sky.js` — 天色:白天 / 黃昏 / 夜晚。開場看訪客自己的時鐘(6–16 白天、17–19 黃昏、
  ...
  **只有夜晚有關燈分級圖**:那時候檯燈是主光,關掉整個房間(含元件與 Allen)會一起暗。
  白天與黃昏關燈只有檯燈自己的像素會變(燈罩不再發亮、燈身暗一階),房間一動也不動
  —— 房間亮的時候關掉一盞桌燈,房間本來就不會變暗, 房間亮的時候關掉一盞桌燈,不會讓整個房間暗一階,
  任何放射狀的暗場都會被看成憑空多出來的黑色光暈,收得再小都一樣(收到 143px 還是
  看得出來)。桌面那圈暖光橢圓也一樣只在夜裡出場。

  重量:白天 + 開燈是原圖本身,**零額外請求**;黃昏 37KB、夜晚 48KB、夜晚的關燈圖 25KB,
  ...
  那圈深色會留在底板上不動,物件一擺動就變成雙重輪廓。差異門檻壓在 15(原本 34):
  34 會把物件的柔邊與軟陰影留在底板上,擺動時那一圈淡淡的輪廓會自己現形;
  再低到 11 就開始淹進牆面的重畫雜訊。
