...
- `allen-sky.js` — 天色:白天 / 黃昏 / 夜晚。開場看訪客自己的時鐘(6–16 白天、17–19 黃昏、
  ...
  窗格要變霓虹)。檯燈與螢幕是自己會發光的東西,所以它們那一層疊在分級**之上** ——
  夜裡它們不該跟著環境一起暗,反而該變成主角。關燈也是分級圖(每個時段一張),
  所以夜裡關檯燈的落差比白天大得多,而且元件與 Allen 會一起暗下來。

  重量:白天 + 開燈是原圖本身,**零額外請求**;黃昏 49KB、夜晚 39KB、三張關燈圖共 16KB,
  全部用到才載。舊的 `stage-off.webp`(91KB)已經被關燈分級圖(5KB)取代。
